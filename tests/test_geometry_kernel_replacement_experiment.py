...
from opal2 import geometry_kernel_replacement_experiment as experiment
...
def fixture_model(mode):
    ...
    bank = ReplacementBasisBank.fit(x,chem,mask,ids,ids[fit],
    ...
    model = GeometryKernelReplacementMean(base,bank,mode=mode,hidden_dim=8,kan_hidden_dim=5)
...
def test_stage_trains_new_branch_preserves_hr_and_reloads_actual_epoch(tmp_path,monkeypatch,mode):
    ...
    assert completion['parameter_counts']['trainable']==sum(p.numel() for p in result.parameters() if p.requires_grad)
    assert completion['parameter_counts']['total']==sum(p.numel() for p in result.parameters())
