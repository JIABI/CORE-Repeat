"""Frozen HR plus matched chemistry–phenotype kernel branches on opened DEV.

The first stage reports both branches at epoch 10, not the test-best checkpoint.
...
def prepare(output, reference):
    ...
    shutil.copy2(PROJECT/'protocols/historical/GEOMETRY_KERNEL_PLAN_20260914.md', root/'PROTOCOL.md')
    ...
    manifest = dict(created_utc=now(), reference_run=str(reference),
        ...
        checkpoint_policy='epoch10 for both branches; validation-best is supplementary only',
    ...
    from .geometry_kernel_summary import _validate_manifest
    _validate_manifest(manifest)
    write_json(root/'run_manifest.json', manifest)
    event(root, 'PREPARED', n=639, branch_fits=10, stage_epochs=10)
    return manifest
...
def load_run(output):
    ...
    if tuple(manifest['arms']) != ARMS:
        raise ValueError('The three declared arms changed')
...
def execute_fold(root, manifest, ds, record, grams, raw_u, gains):
    from .geometry_kernel import DescriptorBank, GeometryKernelMean
    ...
    bank = DescriptorBank.fit(x, ds.chem, ds.chem_mask, ds.ids, ds.ids[fit].tolist(),
    ...
    for arm, mode in (('B_GENERIC','generic'), ('C_STRUCTURED','structured')):
        torch.manual_seed(training_seed)
        model = GeometryKernelMean(hr, bank, mode=mode, incremental_penalty=CONFIG['incremental_penalty'],
...
def execute(output):
    ...
    started = time.monotonic()
    event(root, 'RUNNING', stage_epochs=10, branch_fits=10)
    try:
        with threadpool_limits(limits=CONFIG['threads']):
            ...
            from .geometry_kernel_summary import summarize
            summarize(root)
...
def launch(output):
    ...
    command = [manifest['python_executable'], '-u', '-m', 'opal2.geometry_kernel_experiment',
